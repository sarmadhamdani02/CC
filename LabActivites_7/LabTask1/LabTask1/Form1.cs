...
namespace LabTask1
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeGrammar()
        {
            ...
            grammarRules.Add("B", new List<string> { "d", "?" });
                    ...
                    foreach (var production in grammarRules[nonTerminal])
                    {
                        ...
                        foreach (var symbol in production)
                        {
                            ...
                            else // Non-terminal
                            {
                                ...
                                {
                                    if (!firstSets[nonTerminal].Contains(firstSymbol) && firstSymbol != "?")
                                    {
                                ...
                                // If the first set contains ?, we need to continue looking at the next symbol
                                if (!firstSets[symbolStr].Contains("?"))
                                {
                                    break;
                                }
                            }
                            ...
                            if (char.IsUpper(currentSymbol[0])) // Non-terminal symbol
                            {
                                ...
                                else // Look ahead symbol
                                {
                                    ...
                                    else // Non-terminal symbol
                                    {
                                        ...
                                        {
                                            if (firstSymbol != "?" && !followSets[currentSymbol].Contains(firstSymbol))
                                            {
                                        ...
                                        // If the next symbol can derive ?, add Follow(nonTerminal) to Follow(currentSymbol)
                                        if (firstSets[nextSymbol].Contains("?"))
                                        {
                                            foreach (var followSymbol in followSets[nonTerminal])
                                            {
                                                if (!followSets[currentSymbol].Contains(followSymbol))
